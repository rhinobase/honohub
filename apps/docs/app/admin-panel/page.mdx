...
The Admin Panel will be available at `http://localhost:5173`. You can now start managing your HonoHub project using the Admin Panel. Now one issue is that the API server is running on a different port (in this case on port 3000) or it could be deployed on a domain, So you will need to do the following:
