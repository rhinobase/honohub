...
We are using `@rafty/ui` as our component lib. You can customize the Tailwind CSS configuration to match your project's design system, just change `primary: colors.sky` to your desired color palette. Follow the [Rafty UI documentation](https://rafty.rhinobase.io/) for more information.
...
The Admin Panel will be available at `http://localhost:5173`. You can now start managing your HonoHub project using the Admin Panel. Now one issue is that the API server is running on a different port, So you will need to do the following:
...
The reason for why we separated the API server and the Admin Panel is that you can run and deploy your Hono app in the prefered runtime and serve the Admin Panel as a static site. This way you can deploy the Admin Panel to a CDN or serve it using `serveStatic` middleware in your API server.
...
Go crazy!import ts from "typescript";
